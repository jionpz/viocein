...
use crate::voice_intent::{
    SpeechLanguageMode, VoiceIntent, VoiceIntentKind, VoiceIntentRouter, VoiceMode,
...
fn should_use_byok(config: &storage::AppConfig, llm_api_key: &str) -> bool {
    if !crate::llm::is_supported_provider(&config.llm_provider) {
        return false;
    }
    let base_url = config.llm_base_url.trim().to_string();
    if base_url.trim().is_empty() || config.llm_model.trim().is_empty() {
        return false;
    }
    if !crate::llm::has_usable_provider_credentials(&config.llm_provider, llm_api_key) {
        return false;
    }
    crate::llm::validate_provider_base_url(&config.llm_provider, &base_url).is_ok()
}
...
async fn answer_question(
    ...
    Err(AppError::Config(
        "Configure the company LLM or local Ollama provider before using Ask.".to_string(),
    ))
}
...
async fn ask_via_byok(
    ...
    let base_url = config.llm_base_url.trim().to_string();
    crate::llm::validate_provider_base_url(&config.llm_provider, &base_url)?;

    let api_kind = crate::llm::protocol::detect_api_kind(&config.llm_provider, &base_url);
    let url = crate::llm::protocol::chat_endpoint(&config.llm_provider, &base_url)?;
    let request = client
        .post(url)
    ...
    let request =
        crate::llm::protocol::apply_auth_headers(request, &config.llm_provider, &base_url, api_key);
...
pub(crate) async fn start_reserved_ask_dictation(
    ...
    let result = async {
        ...
        // Managed cloud STT is removed from the internal build.
...
mod tests {
    ...
    #[test]
    fn company_ask_body_uses_openai_compatible_shape() {
        let config = storage::AppConfig {
            llm_provider: crate::llm::COMPANY_PROVIDER.to_string(),
            llm_base_url: "https://llm.corp.example/v1".to_string(),
            llm_model: "company-model".to_string(),
            ..storage::AppConfig::default()
        };

        let body = build_byok_ask_body_for_config(
            &config,
            &config.llm_base_url,
    ...
    #[test]
    fn ask_dictation_stt_config_uses_local_provider_and_multi_language() {
        let config = storage::AppConfig {
    ...
    #[test]
    fn provider_ask_errors_are_short_and_actionable() {
